 ...
 * @param toRem
 */
export function useBreakpoint(bpQuery, defaultState = false, toRem = true) {
...
export const getBreakpointQueryObject = (
  ...
  if (!isNaN(bpQuery)) {
    ...
    let bp = isNaN(bpQueryParts[0]) ? bpQueryParts[0] : Number(bpQueryParts[0]);
    ...
    if (toRem) {
      bp = bp > 0 ? parseFloat(rem(bp)) : 0;
      bpMax = bpMax > 0 ? parseFloat(rem(bpMax)) : 0;
    }
